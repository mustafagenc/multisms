  ...
  SendAsync(MessageBody message,
    ...
    try {
      ...
      using var request = new HttpRequestMessage(
          HttpMethod.Post,
          new UriBuilder(_options.BaseUrl) { Path = "v1/send-sms/json" }.Uri);
      using var jsonContent =
          new StringContent(JsonConvert.SerializeObject(CreateMessage(message)),
...
public partial class IletiMerkeziProvider {
  ...
  public IletiMerkeziProvider(HttpClient httpClient,
                              IletiMerkeziProviderOptions options) {
    if (options is null) {
  ...
  private static SendingResult BuildResultObject(HttpResponseMessage result) {
    return result.IsSuccessStatusCode
               ...
               : SendingResult.Failure(Name).AddError(new SendingError(
  ...
  public IletiMerkeziMessage CreateMessage(MessageBody message) {
    ...
    var key = keyProviderData.IsEmpty() ? _options.Key
                                        : keyProviderData.GetValue<string>();
    var hash = hashProviderData.IsEmpty() ? _options.Hash
                                          : hashProviderData.GetValue<string>();
    var orginator = orginatorProviderData.IsEmpty()
                        ? _options.Orginator
                        : orginatorProviderData.GetValue<string>();

    var option = new IletiMerkeziMessage { request = new Request {
      authentication = new Authentication { key = key, hash = hash },
      order =
          new Order {
            sender = orginator,
            message =
                new Message {
                  ...
                  receipts =
                      new Receipts { number = new List<string> { message.To } }
                }
          }
    } };
