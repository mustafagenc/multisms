...
public class SmsService : ISmsService {
  ...
  public Task<SendingResult>
  SendAsync(MessageBody message, string providerName,
            CancellationToken cancellationToken = default) {
    if (providerName is null) {
      throw new ArgumentNullException(nameof(providerName));
    }
  ...
  public Task<SendingResult>
  SendAsync(MessageBody message, ISmsProvider provider,
            CancellationToken cancellationToken = default) {
    if (message is null) {
      throw new ArgumentNullException(nameof(message));
    }
  ...
  public SmsService(IEnumerable<ISmsProvider> providers,
                    MultiSmsServiceOptions options) {
    if (providers is null) {
      throw new ArgumentNullException(nameof(providers));
    }

    if (!providers.Any()) {
      throw new ArgumentException("En az bir adet saglayici girmelisiniz.",
                                  nameof(providers));
    }
    ...
    if (message.Originator is null) {
      if (Options.DefaultOrginator is null) {
        throw new ArgumentException(
            $"{typeof(MessageBody).FullName} orginator bilgisi yok.");
      }
