  ...
  SendAsync(MessageBody message,
    ...
    try {
      ...
      using var request = new HttpRequestMessage(
          HttpMethod.Post,
          new UriBuilder(_options.BaseUrl) { Path = "sms/send/xml" }.Uri);
...
public partial class NetGsmProvider {
  ...
  private static SendingResult BuildResultObject(HttpResponseMessage result) {
    ...
    if (code == "00" || code == "01" || code == "02") {
      ...
      return SendingResult.Failure(Name).AddError(new SendingError(
          "20",
          "Mesaj metninde ki problemden dolayı gönderilemediğini veya standart maksimum mesaj karakter sayısını geçti."));
    } else if (code == "30") {
      return SendingResult.Failure(Name).AddError(new SendingError(
          "30",
          "Geçersiz kullanıcı adı , şifre veya kullanıcınızın API erişim iznininiz bulunmamakta."));
    } else if (code == "40") {
      return SendingResult.Failure(Name).AddError(new SendingError(
          "40",
          "Mesaj başlığınızın (gönderici adınızın) sistemde tanımlı değil."));
    } else if (code == "70") {
      return SendingResult.Failure(Name).AddError(new SendingError(
          "70",
          "Hatalı sorgulama. Gönderdiğiniz parametrelerden birisi hatalı veya zorunlu alanlardan birinin eksik."));
    } else {
      return SendingResult.Failure(Name).AddError(
          new SendingError("80", "Bilinmeyen bir hata oluştu"));
    }
  }
  ...
  public NetGsmMessage CreateMessage(MessageBody message) {
    ...
    option.Header =
        new Header { Company = new Company { Dil = "TR", Text = "Netgsm" },
                     Usercode = username, Password = password, Type = "1:n",
                     Msgheader = orginator };

    option.Body = new Body { Msg = message.Content, No = message.To };
